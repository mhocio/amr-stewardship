...
public class FratTableService {
    ...
    public SeveralFratTablesResponse getTable(FratRequest fratRequest, Boolean... flags) {
        ...
        Boolean sortTables = flags.length > 1 ? flags[1] : true;
        ...
        LinkedHashMap<HashSet<String>, Double> resultSum2 = new LinkedHashMap<>();
        ...
        for (String multipleAntibiotics: severalAntibioticsFRAT.getAntibiotics()) {
            hashMap.put(multipleAntibiotics, 0.0);
        }
        ...
        for (int i = 0; i < resultSum2.size(); i++) {
            ...
            hashMap.put(multipleAntibioticsList.get(i).toString(), resultSum2.get(multipleAntibioticsList.get(i)));
        }
        rows2.add(toAdd2);
        severalAntibioticsFRAT.setResults(hashMap);
...
//        rows = sortedRowsByBacterias;
        ...
        for (int i = 0; i < resultSum.size(); i++) {
            toAdd.add("");
            toAdd.add(String.format("%.2f", resultSum.get(antibiotics.get(i))));
        }
        rows.add(toAdd);
        res.setRows(rows);

        if (sortTables) {
            res.sortByAntibiotics(bacteriaFrequency);
            severalAntibioticsFRAT.sortByAntibiotics(bacteriaFrequency);
        }
